...
public class TextUI {
    ...
    private void start() throws ClassNotFoundException, SQLException {
        while (true) {
            ...
            List<Deck> tietokannassa = dDao.findAll();
            System.out.println("Tällä hetkellä tietokannassa seuraavat pakat: ");
            System.out.println("id| name");
            for (int i = 0; i < tietokannassa.size(); i++) {
                System.out.println(tietokannassa.get(i).id + "| " + tietokannassa.get(i).name);
            }
            System.out.println("");
                ...
                for (int i = 0; i < pelaaja.size(); i++) {
                    ...
                    for (int j = 0; j < pelaaja2.size(); j++) {
                        ...
                        if (mDao.findOne(d1id, d2id) == null) {
                            ...
                            System.out.println("");
                            System.out.println("Syötä pakan " + d1.name + " winrate pakkaa " + d2.name + " vastaan (esim 50): ");
                            Integer winrateInt = lukija.nextInt();
                            Double winrate = 1.0 * winrateInt / 100;
                            while (winrate < 0 || winrate > 1) {
                                System.out.println("Väärä arvo, syötä uudelleen.");
            ...
            if (mahdollinenBan.size() > 0) {
                ...
                if (conquest) {
                    laskenta.parasBanConquest(player1, mahdollinenBan);
                } else {
                    laskenta.parasBanLHS(player1, mahdollinenBan);
                }
            } else {
                Calculator laskenta = new Calculator(player1, player2);
                if (conquest) {
